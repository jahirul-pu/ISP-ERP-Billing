...
async function main() {
  ...
  // ─── Create Sample Areas, Zones, POPs ─────────
  const area1 = await prisma.area.upsert({
    where: { name: 'Uttara' },
    update: {},
    create: { name: 'Uttara', code: 'UTT' },
  });

  const area2 = await prisma.area.upsert({
    where: { name: 'Mirpur' },
    update: {},
    create: { name: 'Mirpur', code: 'MIR' },
  });

  const zone1 = await prisma.zone.upsert({
    where: { name_areaId: { name: 'Sector 1', areaId: area1.id } },
    update: {},
    create: { name: 'Sector 1', code: 'S1', areaId: area1.id },
  });

  const zone2 = await prisma.zone.upsert({
    where: { name_areaId: { name: 'Sector 3', areaId: area1.id } },
    update: {},
    create: { name: 'Sector 3', code: 'S3', areaId: area1.id },
  });

  const zone3 = await prisma.zone.upsert({
    where: { name_areaId: { name: 'Section 10', areaId: area2.id } },
    update: {},
    create: { name: 'Section 10', code: 'S10', areaId: area2.id },
  });

  await prisma.pop.upsert({
    where: { name_zoneId: { name: 'POP-UTT-S1-01', zoneId: zone1.id } },
    update: {},
    create: { name: 'POP-UTT-S1-01', code: 'P001', zoneId: zone1.id },
  });

  await prisma.pop.upsert({
    where: { name_zoneId: { name: 'POP-UTT-S3-01', zoneId: zone2.id } },
    update: {},
    create: { name: 'POP-UTT-S3-01', code: 'P002', zoneId: zone2.id },
  });

  await prisma.pop.upsert({
    where: { name_zoneId: { name: 'POP-MIR-S10-01', zoneId: zone3.id } },
    update: {},
    create: { name: 'POP-MIR-S10-01', code: 'P003', zoneId: zone3.id },
  });

  console.log('  ✅ Created 2 areas, 3 zones, 3 POPs');

  // ─── Create Sample Packages ───────────────────
  const packages = await Promise.all([
    prisma.package.upsert({
      where: { name: '10 Mbps Home' },
      update: {},
      create: {
        name: '10 Mbps Home',
        bandwidth: '10Mbps',
        price: 800,
        mikrotikProfile: '10Mbps',
        description: 'Standard home internet package',
      },
    }),
    prisma.package.upsert({
      where: { name: '20 Mbps Home' },
      update: {},
      create: {
        name: '20 Mbps Home',
        bandwidth: '20Mbps',
        price: 1200,
        mikrotikProfile: '20Mbps',
        description: 'Premium home internet package',
      },
    }),
    prisma.package.upsert({
      where: { name: '30 Mbps Home' },
      update: {},
      create: {
        name: '30 Mbps Home',
        bandwidth: '30Mbps',
        price: 1500,
        mikrotikProfile: '30Mbps',
        description: 'Ultra home internet package',
      },
    }),
    prisma.package.upsert({
      where: { name: '50 Mbps Corporate' },
      update: {},
      create: {
        name: '50 Mbps Corporate',
        bandwidth: '50Mbps',
        price: 3000,
        mikrotikProfile: '50Mbps',
        description: 'Standard corporate package',
      },
    }),
    prisma.package.upsert({
      where: { name: '100 Mbps Corporate' },
      update: {},
      create: {
        name: '100 Mbps Corporate',
        bandwidth: '100Mbps',
        price: 5000,
        mikrotikProfile: '100Mbps',
